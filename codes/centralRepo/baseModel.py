...
"""
     ...
     This class needs following things to run:
         ...
         2. trainData, testData and validateData -> these should be eegDatasets
             ...
             Each sample of these datasets should b
             e a dictionary with two
...
class baseModel():
    ...
    classes=classes, sampler=sampler
        ...
        if testData is not None:
            pred, act, l = self.predict(testData, sampler=sampler, lossFn_cls=lossFn, lossFn_icp=loss_icp, lossFn_isp=loss_isp)
                ...
                if continueAfterEarlystop:
                    if not earlyStopReached:
                        ...
                        modifiedStop = {'c': {'Or': {'c1': {'MaxEpoch': {'maxEpochs': 300, 'varName' : 'epoch'}},
                                               'c2': {'LessThan': {'minValue' : monitors['valLoss'], 'varName': 'trainLoss'}} } }}
